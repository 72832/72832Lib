...
void RampInit(struct RAMP *controller, int ChangePct=1,int ChangeMsec=8,int MaxPct=75,int MinPct=0,int EndWait=200,Correction=1){
...
int RampCompute(struct RAMP *controller,double Distance,int Feedback){
    ...
    // Is it there yet?
    if(std::abs(Feedback)<std::abs(Distance)){
